...
import { FileText, Download, Eye, Edit, AlertCircle } from 'lucide-react';
...
const InvoiceManager = ({ order }) => {
  ...
  const createInvoice = async (orderId, items, includeCgst = true, includeSgst = true) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${BASE_URL}/api/v1/invoices/create`, {
        ...
        body: JSON.stringify({ 
          orderId, 
          items,
          includeCgst,  // Pass CGST inclusion preference
          includeSgst   // Pass SGST inclusion preference
        })
      });
  ...
  const editInvoice = async (invoiceId, updateData) => {
    try {
      const token = localStorage.getItem("token");
  ...
  // Only show for accounts_paid or order_completed orders
