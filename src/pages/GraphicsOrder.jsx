...
import OrderGraphicsDetails from '../components/GraphicsOrder/OrderGraphicsDetails'; // New component import
...
const GraphicsOrders = () => {
  ...
  const handleStatusUpdate = async (workQueueId, status) => {
    try {
      ...
      setTimeout(() => setUpdateStatus(prev => ({ ...prev, success: null })), 3000);
    } catch (error) {
      console.error("Error updating status:", error);
      setUpdateStatus({ loading: false, error: error.message, success: null });
      setTimeout(() => setUpdateStatus(prev => ({ ...prev, error: null })), 5000);
    }
  };
      ...
      <div className="bg-white shadow-lg rounded-xl overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="p-8 text-center">
            ...
            <button onClick={fetchOrders} className="mt-4 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg transition-colors text-sm">Try Again</button>
          </div>
                  ...
                  <tr key={order._id} className="hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => handleOrderClick(order)}>
                    ...
                    <td className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-right text-xs sm:text-sm font-medium">
                      <div className="flex items-center justify-end space-x-2">
                        <button
                          onClick={(e) => { e.stopPropagation(); handleFileUploadClick(order._id); }}
                        ...
                        {(order.image && order.image.length > 0) || (order.cadFiles && order.cadFiles.length > 0) ? (
                          <button
                            onClick={(e) => { e.stopPropagation(); handleViewFilesClick(order); }}
