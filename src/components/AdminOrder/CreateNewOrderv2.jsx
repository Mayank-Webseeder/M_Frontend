...
import { X, Upload, Users, PackageCheck, ChevronDown, ChevronUp } from "lucide-react";
...
const CreateNewOrder = ({ onClose, addOrder }) => {
    ...
    const handleChange = (e) => {
        ...
        if (name === "customer") {
            ...
            // Clear selected customer if user is typing
    ...
    const handleCustomerDataChange = (e) => {
        ...
        // Don't allow editing if customer is selected from existing list
    ...
    const handleSelectCustomer = (customer) => {
        ...
        // Fill customer data with selected customer info
        setCustomerData({
            ...
            address: {
                ...
                additionalDetail: customer.address?.additionalDetail || ""
            }
        });

        // Show address form if customer has address
        if (customer.address && (customer.address.street || customer.address.city || customer.address.state || customer.address.pincode)) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!customerData.firstName || !customerData.lastName || !customerData.email || !customerData.phoneNo) {
            toast.error("Please fill in all required customer details or select an existing customer");
            setLoading(false);
            return;
        }
    ...
    const isFieldDisabled = selectedCustomer !== null;
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl max-h-screen overflow-hidden flex flex-col">
                ...
                <div className="overflow-y-auto p-4 flex-grow">
                    <form onSubmit={handleSubmit} encType="multipart/form-data" className="space-y-6">
                        {/* Customer Search and Information */}
                        <div className="border-b pb-6">
                            ...
                            <div className="relative mb-4">
                                <label className="block text-sm font-medium text-gray-700 mb-1">
                                    Search Customer
                                    <span className="text-xs text-gray-500 ml-1">(Start typing to search existing customers)</span>
                                </label>
                                <div className="relative">
                                    ...
                                    <input
                                        type="text"
                                        name="customer"
                                        placeholder="Search customer name or create new..."
                                ...
                                {showCustomerDropdown && filteredCustomers.length > 0 && (
                                    <div className="absolute w-full bg-white border rounded-md shadow-lg mt-1 z-20 max-h-48 overflow-y-auto">
                                        <div className="p-2 bg-gray-50 border-b">
                                            <span className="text-xs font-medium text-gray-600">Select existing customer:</span>
                                        </div>
                                        {filteredCustomers.map((customer) => (
                                            <div key={customer._id}
                                                className="p-3 cursor-pointer hover:bg-indigo-50 transition-colors border-b last:border-b-0"
                                            ...
                                            >
                                                ...
                                                <div className="text-xs text-gray-500">{customer.phoneNo}</div>
                                            </div>
                                        ))}
                                    </div>
                                )}
                                ...
                                <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-md">
                                    <div className="flex items-center">
                                        ...
                                        <span className="text-sm font-medium text-green-800">
                                            Existing customer selected: {selectedCustomer.firstName} {selectedCustomer.lastName}
                                        </span>
                                    </div>
                                    <div className="text-xs text-green-600 mt-1">
                                        Customer details are auto-filled and cannot be edited
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className={`w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 ${isFieldDisabled ? 'bg-gray-100 text-gray-600 cursor-not-allowed' : ''
                                            }`}
                                        required
                                    />
                                </div>
                                ...
                                <div className="md:col-span-2">
                                    <button
                                        type="button"
                                        onClick={() => !isFieldDisabled && setShowAddressForm(!showAddressForm)}
                                        disabled={isFieldDisabled}
                                        className={`flex items-center gap-2 text-sm font-medium mb-2 ${isFieldDisabled
                                            ? 'text-gray-500 cursor-not-allowed'
                                            : 'text-indigo-600 hover:text-indigo-800'
                                            }`}
                                    >
                                        {showAddressForm ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
                                        Address Details {isFieldDisabled ? '(Auto-filled)' : '(Optional)'}
                                    </button>
